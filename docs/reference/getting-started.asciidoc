...
* `hits.sort` - 排序结果的关键字（如果按score排序则此字段不存在）
...
for more details.
 ...
 `hits.total` 参数的由请求参数 `track_total_hits`控制，当设置为true时，请求将准确的跟踪总命中数（"relation": "eq"）。默认为10,000 这意味着总命中数被准确地跟踪到10,000文档。您可以通过显式设置track_total_hits为true 来强制进行准确计数。有关详细信息，请参阅<<search-request-track-total-hits, request body>>文档。

Here is the same exact search above using the alternative request body method:
以下是使用请求正文方法替代的上述搜索，结果完全相同：
...
这里的不同之处在于，我们不是在URI中传入q=*，而是向 `_search` API提供JSON样式的查询请求体。我们将在下一节讨论这个JSON查询。
...
Elasticsearch提供了一种JSON样式的特定领域的语言，可用于执行查询。这被称为{ref}/query-dsl.html[Query DSL]。查询语言非常全面，乍一看可能令人生畏，但实际学习它的最佳方法是从一些基本示例开始。
...
解析上面的内容，该query部分告诉我们查询定义是什么，match_all部分只是我们想要运行的查询类型。match_all查询仅仅是在指定索引的所有文档中进行搜索。

除了query参数，我们还可以传递其他参数来影响搜索结果。在上面的部分示例中，我们传入了`sort`，这里我们传入size：
...
还有许多其他聚合功能，我们在此不再详述。如果您想进行进一步的实验，{ref}/search-aggregations.html [聚合参考指南]是一个很好的起点。
